...
*   **PyOD ECOD (Empirical Cumulative Distribution):** Для одномерных порогов (pressure_delta, pressure_slope) внедрен метод ECOD. Это позволило уйти от ручного подбора квантилей и "магических чисел", автоматически определяя границы аномалий на основе эмпирического распределения данных.
*   **Удален "самописный" код:**
    ...
    *   Ручной расчет квантилей для порогов детекции.
*   **Внедрен `pyod`:** Теперь модели учатся через `.fit()` и предсказывают через `.decision_function()`.
...
### Отказ от миграции на Darts/sktime
*   **Решение:** Оставить обработку временных рядов на стеке **Pandas (rolling, resample)**.
*   **Причина:** Текущая реализация на Pandas полностью покрывает потребности проекта. Переход на специализированные библиотеки (Darts/sktime) добавил бы лишнюю зависимость и сложность без существенного выигрыша в функциональности для текущих задач (в основном это простые скользящие окна и агрегации).
