"""
Feature importance report for PaAno negermet (НКТ leak) detection.
Adapted from generate_salt_feature_importance_report.py with shorter patches.
"""
import sys
import os
import io
...
def load_intervals():
    intervals = pd.read_csv('db/negermet_intervals.csv', dtype={'well_id': str})
...
def compute_feature_importance(well_df, intervals_df, well_id, device, verbose=True):
    ...
    if len(data) < PATCH_SIZE * 3:
        if verbose:
            print(f'  {well_id}: недостаточно данных ({len(data)} < {PATCH_SIZE * 3})')
        return None

    anomaly_mask = labels == 1.0
    if not anomaly_mask.any():
        if verbose:
            print(f'  {well_id}: нет аномальных точек в данных')
        return None
...
def make_importance_bar_b64(result, top_n=15):
    ...
    colors = ['#c0392b' if v > 0 else '#bdc3c7' for v in df_top['drop_pct']]
    ...
    ax.set_title(f"Скважина {result['well_id']} \u2014 Важность каналов для детекции негерметичности НКТ")
...
def make_top_channels_timeseries_b64(result, intervals_df, top_n=6):
    ...
    ax0 = axes[0]
    ax0.fill_between(ts_pd, 0, result['baseline_scores'], color='#c0392b', alpha=0.25)
    ax0.plot(ts_pd, result['baseline_scores'], color='#c0392b', linewidth=0.6)
    for _, row in wi.iterrows():
    ...
    ax0.set_title(f"Скважина {well_id} \u2014 Вклад топ-{top_n} каналов (негерметичность НКТ)",
...
<head>
    ...
    <style>
        ...
        .positive {{ color: #c0392b; font-weight: bold; }}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='negermet_paano_feature_importance.html')
