...
Usage:
  python build_negermet_dataset.py                  # default 15s
  python build_negermet_dataset.py --freq 2min      # 2-minute grid
  python build_negermet_dataset.py --freq 15s       # 15-second grid
"""
import re
import warnings
...
WELL_FILES = {
    '5271г': 'negermet_anomaly_files/5271г_ЮЯ_ННКТ.xlsx',
    '524':   'negermet_anomaly_files/524_ЮЯ_ННКТ.xlsx',
    '3509г': 'negermet_anomaly_files/3509г_Кустовое_ННКТ.xlsx',
    '1123л': 'negermet_anomaly_files/1123л_Кустовое_ННКТ.xlsx',
    '172г':  'negermet_anomaly_files/172г_Яркое_ННКТ.xlsx',
}

# Normalize parameter names across wells
PARAM_RENAME = {
...
def main(freq='15s'):
    ...
    out_path = Path(f'db/negermet_anomaly_database_{freq_label}.csv')
    result.to_csv(out_path, index=False)
    print(f'\nDatabase: {out_path} ({len(result)} rows)')

    # 3. Build intervals
    idf = build_intervals()
    intervals_path = Path('db/negermet_intervals.csv')
