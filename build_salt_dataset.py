"""
Build salt (солеотложение) dataset from xlsx files.
Produces:
  - db/salt_anomaly_database_<freq>.csv   (interpolated on uniform grid)
  - db/salt_intervals.csv                 (ground truth intervals)

Usage:
  python build_salt_dataset.py                  # default 2min
  python build_salt_dataset.py --freq 15s
"""
import warnings
...
WELL_FILES = {
    '3244г': 'salt_anomaly_files/3244г_Кустовое_Соли.xlsx',
    '3245':  'salt_anomaly_files/3245_Кустовое_Соли.xlsx',
    '149г':  'salt_anomaly_files/149г_ВИК_Соли.xlsx',
    '3269':  'salt_anomaly_files/3269_Кустовое_Соли.xlsx',
    '4039':  'salt_anomaly_files/4039_Дружное_Соли.xlsx',
    '408':   'salt_anomaly_files/408_Дружное_Соли.xlsx',
    '3245(2)': 'salt_anomaly_files/3245(2)_Кустовое_Соли.xlsx',
}
...
def parse_xlsx(well_id, filepath):
    ...
    for sname in wb.sheetnames:
        ...
        # Salt xlsx has header row at row 3 ('Время', 'Значение'), data starts at row 4
        data_start = 3
        if len(rows) > 2:
            r2 = rows[2]
            if r2 and r2[0] and 'время' in str(r2[0]).lower():
                data_start = 3
...
def build_intervals():
    ...
    svod = pd.read_excel('salt_anomaly_files/Сводная информация.xlsx', header=None)
    ...
    for _, row in data.iterrows():
        ...
        data_start = pd.to_datetime(row.iloc[3], errors='coerce')
        data_end = pd.to_datetime(row.iloc[4], errors='coerce')

        # First anomaly interval (cols 6, 7)
        s1 = pd.to_datetime(row.iloc[6], errors='coerce')
        e1 = pd.to_datetime(row.iloc[7], errors='coerce')
        ...
        # Second anomaly interval (cols 8, 9) if present
        if len(row) > 9:
            s2 = pd.to_datetime(row.iloc[8], errors='coerce')
            e2 = pd.to_datetime(row.iloc[9], errors='coerce')
...
def main(freq='2min'):
    print(f'=== Построение датасета Солеотложение (шаг={freq}) ===\n')
    ...
    out_path = Path(f'db/salt_anomaly_database_{freq_label}.csv')
    result.to_csv(out_path, index=False)
    print(f'\nDatabase: {out_path} ({len(result)} rows)')

    idf = build_intervals()
    intervals_path = Path('db/salt_intervals.csv')
