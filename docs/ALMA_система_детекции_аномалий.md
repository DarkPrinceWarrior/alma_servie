...
### 6.4 Доменное предобучение на наборе 3W (Petrobras)

Скважин с разметкой аномалий в проекте немного, и нейросетевому encoder-у этого может не хватать, чтобы уверенно отделить норму от аномалии. Чтобы дать модели больше «нефтяного опыта», используется **доменное предобучение** на открытом промышленном наборе данных **3W (Petrobras)** — это телеметрия нефтяных скважин с девятью типами аномалий.

Идея переноса обучения:

1. Сначала encoder обучается на большом наборе 3W и «запоминает» общие закономерности работы нефтяной скважины.
2. Затем веса этого encoder-а переносятся в систему ALMA: первый слой, привязанный к конкретному набору датчиков, заменяется на «родной» для ALMA, а остальные слои сохраняются.
3. Перенесённый encoder дообучается на нормальных участках скважин ALMA — это и есть финальная модель `paano_shared`.

Что это дало в production-прогоне:

| Аномалия | Эффект доменного предобучения |
|---|---|
| Негерметичность НКТ | Частота ложных срабатываний снизилась с 0.250 до 0.188 в сутки (−25%), меньше повторных стартов |
| Изменение притока | Более раннее срабатывание на обучающих скважинах (задержка меньше примерно на 3 часа) |
| Солеотложение | Без изменений — здесь итоговый score определяется физической веткой, а не нейросетью |

Набор 3W используется именно как **доменный «тренажёр» и эталон для сравнения**, а не как замена данным заказчика. Полный технический отчёт по работе с 3W, перечень экспериментов и метрики по всем девяти классам — в файле `docs/3w_pipeline.md`.
...
Частота ложных срабатываний негерметичности снижена с 0.250 до 0.188 в сутки за счёт доменного предобучения на наборе 3W (см. раздел 6.4).
...
> **Документ обновлён:** 14 мая 2026
> **Версия системы:** ALMA v2.3 (Shared Encoder + физические ветки: Pressure Trend для притока, Salt Deposition для соли; доменное предобучение на наборе 3W Petrobras)
> **Технологический стек:** Python, PyTorch, scikit-learn, matplotlib
> **Связанные документы:** `docs/3w_pipeline.md` — полный отчёт по работе с набором 3W и переносу обучения
