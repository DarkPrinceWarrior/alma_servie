...
| Salym raw pipeline | `alma_service/salym/raw_pipeline.py`, `scripts/salym/build_salym_raw.py` | Оставить изолированно | Нужен для отдельной Salym-задачи, но вынесен в `salym` namespace и не участвует в основном `paano_shared/global` runtime |
| Salym-only reports/packages | `scripts/detection/screen_salym_unlabeled.py`, `scripts/reports/generate_salym_expert_package.py`, `scripts/build_salym_human_pdf.sh`, `docs/salym/*`, `docs/assets/salym_ws1333_gantt.png` | Удалить | Старые экспертные пакеты и human docs не относятся к двум основным детекторам |
