...
### Прогресс 2026-05-20: первый `salt` domain guard

Выполнена первая правка `domain decision layer` для `salt`.

Проблема предыдущей версии: salt verdict принимал слишком простой паттерн
`pressure_post_slope_or_reversal_up`. Это полезно для солей, но без уточнений
может принимать слабый локальный разворот давления как уверенное
солеотложение и неявно смешивать настоящую деградацию ГНО с нормальной
реакцией на изменение частоты.

Новое правило:

```text
для salt уверенно принимаем:
  pressure level-up
  или сильный локальный разворот тренда давления вверх

если частота выросла, а давление всё равно растёт:
  это усиливает salt-кандидат

если частота выросла, а давление падает:
  rejected_frequency_transition
  потому что это ожидаемая режимная реакция, а не salt-подтверждение

если есть только слабый разворот без level-shift и без поддержки:
  uncertain/review

bad_data/regime context сам по себе не отменяет salt,
если pressure pattern сильный или есть поддержка нагрузкой/температурой/дисбалансом
```

Проверка выполнена без переобучения: сохраненные `salt` старты последнего
global benchmark были пропущены через новый classifier.

Результат:

```text
all starts:       accept=14, reject=1, uncertain=7
labelled starts:  accept=13, reject=1
non-labelled:     accept=1,  uncertain=7
```

Ключевые кейсы:

| Скважина | Стало | Причина |
|---|---|---|
| `3244г` первый старт | `salt_candidate / accept` | частота растет, но давление тоже растет; это salt-сигнал |
| `3245(2)` 2025-10-05 | `salt_candidate / accept` | локальный разворот давления вверх после предшествующего снижения |
| `3245(2)` 2025-10-16 | `rejected_frequency_transition` | давление падает на фоне роста частоты; эксперт описывал такой эпизод как нормальную режимную реакцию |
| `1740` большинство blind-стартов | `uncertain / review` | слабый или отрицательный pressure pattern без уверенного salt-подтверждения |

Интерпретация: salt guard теперь ближе к экспертной логике. Он отличает
"давление растет несмотря на частоту" от "давление падает из-за частоты" и не
делает auto-accept на слабом локальном развороте. Как и остальные guard-слои,
