...
| 2026-04-24 | **D. worker** | `<pending>` | src/back/worker/loop.py (claim FOR UPDATE SKIP LOCKED + execute + finalize + graceful shutdown), app/back/worker/Dockerfile (python:3.12 + torch-cpu + research reqs), compose profile heavy; api при DETECTION_MOCK=false больше не запускает subprocess — только вставляет pending; +3 unit-теста |
| | B. detections | | |
| | C. reports | | |
| | D. worker | | |
| | E. auth | | |
