"""
Feature importance report for PaAno salt anomaly detection.

For each well and each channel, measures how much the anomaly score
in the anomaly zone drops when that channel is replaced with its
training-set mean (permutation importance). Higher drop = more important.
"""
import sys
import os
import io
...
def load_salt_intervals():
    intervals = pd.read_csv(DB_DIR / 'salt_intervals.csv', dtype={'well_id': str})
...
def compute_feature_importance(well_df, intervals_df, well_id, device, verbose=True):
    ...
    baseline_anom_score = float(np.mean(baseline_scores[anomaly_mask])) if anomaly_mask.any() else 0.0
    ...
    for ch_idx in range(in_channels):
        ...
        perturbed[:, ch_idx] = 0.0  # mean of normalized = 0

        perturbed_scores = score_full_data(model, perturbed, train_norm, memory_bank, device)
        perturbed_anom_score = float(np.mean(perturbed_scores[anomaly_mask])) if anomaly_mask.any() else 0.0
    ...
    # Per-channel anomaly-zone score timeseries (for per-channel plot)
    per_channel_scores = {}
    for ch_idx in range(in_channels):
        single = np.zeros_like(full_norm)
        single[:, ch_idx] = full_norm[:, ch_idx]
        sc = score_full_data(model, single, train_norm, memory_bank, device)
        per_channel_scores[col_names[ch_idx]] = sc
...
def make_importance_bar_b64(result, top_n=15):
    ...
    colors = ['#8e44ad' if v > 0 else '#bdc3c7' for v in df_top['drop_pct']]
    bars = ax.barh(range(len(df_top)), df_top['drop_pct'].values, color=colors)
    ax.set_yticks(range(len(df_top)))
    ax.set_yticklabels(df_top['channel'].values, fontsize=9)
    ax.invert_yaxis()
    ax.set_xlabel('Снижение PaAno Score при отключении канала, %')
    ax.set_title(f"Скважина {result['well_id']} \u2014 Важность каналов для детекции аномалии")
...
def make_top_channels_timeseries_b64(result, intervals_df, top_n=6):
    ...
    # Top panel: baseline PaAno score
    ax0 = axes[0]
    ts_pd = pd.to_datetime(timestamps)
    ax0.fill_between(ts_pd, 0, result['baseline_scores'], color='purple', alpha=0.25)
    ax0.plot(ts_pd, result['baseline_scores'], color='purple', linewidth=0.6)
    for _, row in wi.iterrows():
    ...
    ax0.set_title(f"Скважина {well_id} \u2014 Вклад топ-{top_n} каналов", fontsize=11, fontweight='bold')
...
def generate_report(source_path, output_path, report_title):
    ...
    all_results = []
    for wid in sorted(salt_df['well_id'].unique()):
        print(f'\nСкважина {wid}:')
...
<head>
    ...
    <style>
        ...
        th {{ background-color: #8e44ad; color: #fff; }}
        tr:nth-child(even) {{ background-color: #f9f3fc; }}
        .positive {{ color: #8e44ad; font-weight: bold; }}
        .negative {{ color: #95a5a6; }}
        .plot-container {{ margin-bottom: 40px; background: #fff; border: 1px solid #ddd;
                          border-radius: 6px; padding: 15px; }}
        img {{ max-width: 100%; height: auto; }}
        .method-box {{ background: #f4ecf7; border-left: 4px solid #8e44ad; padding: 15px 20px;
                       margin: 20px 0; border-radius: 4px; line-height: 1.7; }}
        .method-box b {{ color: #6c3483; }}
    </style>
</head>
<body>
    <h1>{report_title}</h1>
    ...
    <div class="method-box">
        ...
        <ul>
            ...
            <li><b>Около нуля или отрицательное</b> &mdash; канал малозначим или даже вносит шум.</li>
        </ul>
        <p>Для каждой скважины представлены: диаграмма важности каналов и временные ряды
        топ-каналов с наложенными зонами аномалий.</p>
    </div>
"""
    ...
    <div class="plot-container">
        <img src="data:image/png;base64,{bar_b64}" alt="Важность каналов {wid}">
    </div>
"""

        # Timeseries of top channels
        ts_b64 = make_top_channels_timeseries_b64(result, intervals, top_n=6)
        html += f"""
    <div class="plot-container">
        <img src="data:image/png;base64,{ts_b64}" alt="Топ каналы {wid}">
    </div>
"""
    ...
    <table>
        <tr>
            <th>#</th>
            <th>Канал</th>
            <th>Baseline Score (аном. зона)</th>
            <th>Score без канала</th>
            <th>Снижение</th>
            <th>Снижение, %</th>
        </tr>
"""
        ...
        <tr>
            ...
            <td>{ch}</td>
            <td>{v['baseline']:.6f}</td>
            <td>{v['perturbed']:.6f}</td>
            <td class="{cls}">{v['drop']:+.6f}</td>
            <td class="{cls}">{v['pct_drop']:+.1f}%</td>
        </tr>
"""
        html += '    </table>\n'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Отчёт влияния переменных на PaAno детекцию солей')
    parser.add_argument('--source', default=str(DB_DIR / 'salt_anomaly_database_interpolated_2min.csv'))
    parser.add_argument('--output', default=str(REPORTS_DIR / 'salt_paano_feature_importance.html'))
    parser.add_argument('--title', default='Влияние переменных на детекцию солеотложений (PaAno)')
