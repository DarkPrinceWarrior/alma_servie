...
def load_intervals():
    src = DB_DIR / 'pritok_intervals.csv'
    ...
    <div class="method-box">
        ...
        <ol>
            <li><b>Обучение на норме.</b> Исторический префикс ряда используется как эталон нормального
                режима. Для него обучаются две PaAno-модели: короткий патч (32 точки) и длинный патч
                (64 точки). Их скоры объединяются в единый multi-scale score.</li>
            ...
            <li><b>Детекция.</b> Старт определяется каузально: пороги калибруются на префиксе
                (целевой FAR/day), затем применяются robust z-score, EMA и CUSUM.</li>
        </ol>
        <p><b>Особенности для изменения притока:</b> multi-scale конфигурация (32+64 точки)
        на 2-минутной сетке помогает фиксировать начало медленного тренда раньше.</p>
    </div>
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default=str(REPORTS_DIR / 'pritok_paano_report.html'))
    parser.add_argument('--title', default='Детекция изменения притока методом PaAno (AI)')
    parser.add_argument('--paano-results', default=str(RESULTS_DIR / 'pritok_paano_results.csv'))
    parser.add_argument('--paano-scores', default=str(DB_DIR / 'pritok_paano_scores.csv'))
