"""
PaAno report for Негерметичность НКТ.
Mirrors generate_salt_paano_report.py.
"""
import base64
import argparse
import io
...
<head>
    ...
    <style>
        ...
        .method-box {{ background: #fdf2f2; border-left: 4px solid #c0392b; padding: 15px 20px;
                       margin: 20px 0; border-radius: 4px; line-height: 1.7; }}
        .method-box b {{ color: #922b21; }}
...
<body>
    ...
    <div class="method-box">
        ...
        <ol>
            <li><b>Обучение на норме.</b> Исторический префикс ряда используется как эталон нормального
                режима. Из его патчей строится банк нормальных эмбеддингов PaAno.</li>
            ...
            <li><b>Детекция.</b> Старт аномалии определяется каузально: пороги калибруются на
                историческом префиксе (целевой FAR/day), затем применяются robust z-score, EMA и CUSUM.</li>
        </ol>
        <p><b>Особенности для негерметичности НКТ:</b> используется патч 128 точек (32 минуты на сетке 15с)
        с каузальной калибровкой порога для устойчивого online-детекта резких аномалий.</p>
    </div>
...
"""
    ...
    for _, row in intervals.iterrows():
        ...
        if pd.notna(det):
            if row['start_date'] <= det <= row['end_date']:
                status, cls = 'Обнаружено', 'status-detected'
            else:
                status, cls = 'Вне интервала', 'status-notfound'
        else:
...
"""
    ...
    for i, (_, row) in enumerate(intervals.iterrows()):
        ...
        title = f"Скважина {wid} \u2014 негерметичность НКТ, интервал {idx}"
        x_min = data_start
        x_max = data_end
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default=str(REPORTS_DIR / 'negermet_paano_report.html'))
    parser.add_argument('--title', default='Детекция негерметичности НКТ методом PaAno (AI)')
    parser.add_argument('--paano-results', default=str(RESULTS_DIR / 'negermet_paano_results.csv'))
    parser.add_argument('--paano-scores', default=str(DB_DIR / 'negermet_paano_scores.csv'))
