...
from alma_service.paths import DB_DIR, REPORTS_DIR, RESULTS_DIR, ensure_parent
...
def load_paano_detected_points(path):
    src = Path(path)
    if not src.exists():
        return {}
...
def create_plot_base64(well_data, start_dt, end_dt, paano_detected_dt, title,
    ...
    series_map = [
        (SALT_PRESSURE_COL, 'Давление на приёме насоса, кгс/см\u00b2', 'tab:blue'),
        (SALT_FREQ_COL, 'Выходная частота, Гц', 'tab:orange'),
    ]
    ...
    for ax, (col, label, color) in zip(axes[:2], series_map):
        ...
        ax.axvspan(start_dt, end_dt, color='tab:red', alpha=0.12, label='Интервал аномалии')
        ax.axvline(start_dt, color='tab:green', linestyle='-', linewidth=1.0, label='Факт. начало аномалии')
        if pd.notna(x_min) and pd.notna(x_max) and x_min < x_max:
            ax.set_xlim(x_min, x_max)
        if pd.notna(paano_detected_dt):
            ax.axvline(paano_detected_dt, color='purple', linestyle='--', linewidth=1.3,
                       label='PaAno: обнаружено')
        ax.set_ylabel(label)
    ...
    if has_paano:
        ...
        ax_p.fill_between(score_df['timestamp'], 0, score_df['paano_score'],
                          color='purple', alpha=0.25)
        ax_p.plot(score_df['timestamp'], score_df['paano_score'],
                  color='purple', linewidth=0.6, label='PaAno Score')
        ax_p.axvspan(start_dt, end_dt, color='tab:red', alpha=0.12, label='Интервал аномалии')
        ax_p.axvline(start_dt, color='tab:green', linestyle='-', linewidth=1.0, label='Факт. начало аномалии')
        if pd.notna(paano_detected_dt):
            ax_p.axvline(paano_detected_dt, color='purple', linestyle='--', linewidth=1.3,
                         label='PaAno: обнаружено')
...
def generate_html(
    ...
    detected_count = sum(1 for k in paano_detected_map if pd.notna(paano_detected_map[k]))
...
"""
    ...
    for _, row in intervals.iterrows():
        ...
        if pd.notna(paano_detected):
            if row['start_date'] <= paano_detected <= row['end_date']:
                status = 'Обнаружено'
                status_cls = 'status-detected'
            else:
                status = 'Вне интервала'
                status_cls = 'status-notfound'
        else:
            status = 'Не найдено'
            status_cls = 'status-notfound'

        det_str = paano_detected.strftime('%Y-%m-%d %H:%M') if pd.notna(paano_detected) else '&mdash;'
        ds = row['data_start'].strftime('%Y-%m-%d') if pd.notna(row['data_start']) else '&mdash;'
        de = row['data_end'].strftime('%Y-%m-%d') if pd.notna(row['data_end']) else '&mdash;'
...
"""
    ...
    for i, (_, row) in enumerate(intervals.iterrows()):
        ...
        print(f'  График {i + 1}/{total}: скв. {well_id}, интервал {interval_idx}')
        well_data = salt_df[salt_df['well_id'] == well_id]
        if well_data.empty:
            html += f"""
    <div class="plot-container">
        <h3>Скважина {well_id} (интервал {interval_idx})</h3>
...
"""
        ...
        title = f"Скважина {well_id} \u2014 интервал солеотложения {interval_idx}"
        ...
        b64_img = create_plot_base64(plot_df, start_dt, end_dt, paano_detected, title,
                                      x_min, x_max, paano_scores_df=paano_sc)
        if b64_img is None:
            html += f"""
    <div class="plot-container">
        <h3>{title}</h3>
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', default=str(DB_DIR / 'salt_anomaly_database_2min.csv'),
                        help='CSV с данными скважин')
    parser.add_argument('--output', default=str(REPORTS_DIR / 'salt_paano_report.html'),
                        help='Путь к выходному HTML')
    parser.add_argument('--title', default='Детекция солеотложений методом PaAno (AI)',
                        help='Заголовок отчёта')
    parser.add_argument('--paano-results', default=str(RESULTS_DIR / 'salt_paano_results.csv'),
                        help='CSV с результатами PaAno детекции')
    parser.add_argument('--paano-scores', default=str(DB_DIR / 'salt_paano_scores.csv'),
    ...
    generate_html(
        ...
        paano_results_path=args.paano_results,
        paano_scores_path=args.paano_scores,
    )
