...
Usage:
  python scripts/datasets/build_salt_dataset.py                  # default 2min
  python scripts/datasets/build_salt_dataset.py --freq 15s
"""
import sys
import itertools
import warnings
...
def parse_xlsx(well_id, filepath):
    ...
    for sname in wb.sheetnames:
        ...
        times = []
        values = []
        if third_row and third_row[0] is not None:
...
def build_intervals():
    ...
    for _, row in data.iterrows():
        ...
        if 'соли' not in anom_type.lower():
            continue
        ...
        # 7/8 -> first anomaly interval, 9/10 -> second anomaly interval.
        ...
        if pd.notna(s1) and pd.notna(e1):
            intervals.append({
                ...
                'data_end': data_end,
                'split': split_for_well('salt', well_id),
            })
...
def main(freq='2min'):
    ...
    out_path = DB_DIR / f'salt_anomaly_database_{freq_label}.csv'
    result.to_csv(out_path, index=False)
    print(f'\nDatabase: {out_path} ({len(result)} rows)')

    idf = build_intervals()
    intervals_path = DB_DIR / 'salt_intervals.csv'
