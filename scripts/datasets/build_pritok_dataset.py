...
Usage:
  python scripts/datasets/build_pritok_dataset.py                  # default 2min
  python scripts/datasets/build_pritok_dataset.py --freq 15s
"""
import sys
import warnings
    ...
    for _, row in data.iterrows():
        ...
        if pd.notna(s1) and pd.notna(e1):
            intervals.append({
                ...
                'data_end': data_end,
                'split': split_for_well('pritok', well_id),
            })
...
def main(freq='2min'):
    ...
    out_path = DB_DIR / f'pritok_anomaly_database_{freq_label}.csv'
    result.to_csv(out_path, index=False)
    print(f'\nDatabase: {out_path} ({len(result)} rows)')

    idf = build_intervals()
    intervals_path = DB_DIR / 'pritok_intervals.csv'
