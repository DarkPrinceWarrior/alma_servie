...
def process_one_instance(
    ...
    try:
        ...
        return (
            InstanceMeta(
                instance_id=path.stem + f"__c{folder_label}",
                path=rel_path,
                folder_label=folder_label,
                event_name=event_name,
                source_type=source_type,
                n_rows=0,
                start_ts=pd.Timestamp("NaT"),
                end_ts=pd.Timestamp("NaT"),
                duration_hours=0.0,
                columns_available=[],
                is_valid=False,
                ...
                has_transient=False,
                has_event=False,
                undesirable_start_ts=None,
                transient_start_ts=None,
                event_start_ts=None,
            ),
            None,
        )
    ...
    if len(resampled) < cfg["preprocess"]["min_points_per_instance"]:
        return (
            InstanceMeta(
                ...
                start_ts=resampled.index[0],
                end_ts=resampled.index[-1],
                duration_hours=(resampled.index[-1] - resampled.index[0]).total_seconds() / 3600.0,
                columns_available=usable,
                is_valid=False,
    ...
    meta = InstanceMeta(
        ...
        has_event=has_event,
        undesirable_start_ts=undesirable,
        transient_start_ts=transient_start,
...
def make_splits(manifest: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    prefer_real_in_test = cfg["splits"]["prefer_real_in_test"]
    valid = manifest[manifest["is_valid"]].copy()
    rows: list[dict] = []
    for label, group in valid.groupby("folder_label"):
        ids = group["instance_id"].tolist()
        real_ids = group[group["source_type"] == "real"]["instance_id"].tolist()
        other_ids = [i for i in ids if i not in real_ids]
        rng.shuffle(real_ids)
        rng.shuffle(other_ids)
        n_total = len(ids)
        n_test = max(int(round(n_total * (1 - train_ratio - val_ratio))), 1) if n_total >= 3 else 1
        n_val = max(int(round(n_total * val_ratio)), 1) if n_total >= 4 else 0
        if prefer_real_in_test and real_ids:
            test_ids = real_ids[: min(n_test, len(real_ids))]
            remaining_real = real_ids[len(test_ids):]
            remaining = remaining_real + other_ids
        else:
            combined = (real_ids + other_ids) if not prefer_real_in_test else other_ids
            test_ids = combined[:n_test]
            remaining = combined[n_test:]
        val_ids = remaining[:n_val]
        train_ids = remaining[n_val:]
        for iid in train_ids:
            rows.append({"instance_id": iid, "folder_label": label, "split": "train"})
        for iid in val_ids:
            rows.append({"instance_id": iid, "folder_label": label, "split": "val"})
        for iid in test_ids:
            rows.append({"instance_id": iid, "folder_label": label, "split": "test"})
...
def main() -> None:
    ...
    parser.add_argument("--limit-classes", default="", help="Optional CSV subset like '0,7' for fast smoke-test")
    ...
    classes_arg = args.limit_classes or args.classes
    target_labels = [int(x) for x in classes_arg.split(",") if x.strip() != ""]
    ...
    for folder_label, frames in per_class_rows.items():
        ...
        if not kept:
            ...
            continue
        df = pd.concat(kept, axis=0, ignore_index=True)
        feature_columns = [c for c in df.columns if c not in ("instance_id", "timestamp", "class_value", "is_normal")]
        common = set(feature_columns)
        for iid in df["instance_id"].unique():
            sub_cols = set(df.columns)
            common &= sub_cols
        kept_columns = sorted([c for c in feature_columns if c in common])
        keep = ["instance_id", "timestamp", "class_value", "is_normal"] + kept_columns
        df = df[keep]
        out_path = out_dir / "features.parquet"
        df.to_parquet(out_path, engine="pyarrow", compression="brotli")
        print(
            f"[class {folder_label}] features  rows={len(df)}  channels={len(kept_columns)}  -> {out_path}",
            flush=True,
        )
    ...
    summary = {
        ...
        "split_counts": splits["split"].value_counts().to_dict() if len(splits) else {},
