...
Uses causal onset detection calibrated on a historical reference window.
"""
...
SEED = 2027
PATCH_SIZE = 128  # 128 x 15s = 32 minutes
NUM_ITERS = 200
BATCH_SIZE = 256
LR = 1e-4
TOP_K = 3
...
REFERENCE_MIN_DAYS = 0.25
TARGET_FAR_PER_DAY = 1.0
MIN_RUN_POINTS = 3
COOLDOWN_HOURS = 4
EMA_ALPHA = 0.10
...
def load_negermet_data(source_path=None):
    if source_path is not None:
        ...
        candidates = [
            DB_DIR / "negermet_anomaly_database_15s.csv",
            DB_DIR / "negermet_anomaly_database_2min.csv",
        ]
...
def load_negermet_intervals():
    src = DB_DIR / "negermet_intervals.csv"
    if not src.exists():
        return pd.DataFrame()
...
def detect_negermet_paano_well(well_df, well_id, verbose=True):
    set_seed()
    ...
    if reference_end_idx < PATCH_SIZE * 2:
        return None, timestamps, "Not enough historical reference data"
    ...
    detail = (
        f"PaAno single-scale(p={PATCH_SIZE},iters={NUM_ITERS},train_ref={reference_end_idx},"
        f"q={thresholds.quantile:.5f},thr_score={thresholds.score_threshold:.3f},"
        f"thr_ema={thresholds.ema_z_threshold:.3f},thr_cusum={thresholds.cusum_threshold:.3f},"
        f"train_time={t_train:.1f}s,starts={len(starts)})"
    )
    if verbose:
...
def _map_predictions_to_intervals(
    ...
    well_intervals,
    detail,
    prestart_tolerance_hours=2,
    early_tolerance_hours=2,
):
...
def run_negermet_paano_detection(
    output_path=RESULTS_DIR / "negermet_paano_results.csv",
    source_path=None,
    verbose=True,
):
    print("=== PaAno Negermet Detection (causal) ===")
    ensure_dir(DB_DIR)
    ...
    for wid in all_wells:
        ...
        out, timestamps, detail = detect_negermet_paano_well(well_data, wid, verbose=verbose)
        if out is None:
        ...
        for ts, sc in zip(timestamps, out["score"]):
            score_rows.append({"well_id": wid, "timestamp": ts, "paano_score": float(sc)})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default=str(RESULTS_DIR / "negermet_paano_results.csv"))
    ...
    if args.well:
        ...
        if well_data.empty:
            ...
            sys.exit(1)
        out, _, detail = detect_negermet_paano_well(well_data, wid, verbose=True)
        starts = [] if out is None else out["starts"]
