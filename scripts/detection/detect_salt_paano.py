...
def load_salt_data(source_path=None):
    if source_path is not None:
        ...
        candidates = [
            DB_DIR / "salt_anomaly_database_2min.csv",
            DB_DIR / "salt_anomaly_database_15s.csv",
            DB_DIR / "salt_anomaly_database_interpolated_2min.csv",
            DB_DIR / "salt_anomaly_database_interpolated.csv",
            DB_DIR / "salt_anomaly_database.csv",
        ]
...
def run_salt_paano_detection(
    output_path=RESULTS_DIR / "salt_paano_results.csv",
    source_path=None,
    mode="dev",
    config_path=DB_DIR / "salt_paano_config.json",
    auto_tune=None,
    verbose=True,
):
    ...
    if mode == "dev":
        ...
        if auto_tune:
            ...
            (DB_DIR / "salt_dev_tuning.json").write_text(
                json.dumps(tune_summary, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
            print(f"Saved tuned config to {config_path}")
            print(f"Saved dev tuning summary to {DB_DIR / 'salt_dev_tuning.json'}")
        else:
    ...
    scores_output = DB_DIR / "salt_paano_scores.csv"
    if score_rows:
    ...
    pred_output = DB_DIR / "salt_paano_predicted_starts.csv"
    if pred_rows:
...
def detect_salt_paano_starts(well_data, intervals_df=None, well_id=None, mode="blind"):
    ...
    cfg = load_or_default_config(DB_DIR / "salt_paano_config.json")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    data, timestamps = prepare_well_matrix(well_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default=str(RESULTS_DIR / "salt_paano_results.csv"))
    parser.add_argument("--source", type=str, default=None)
    parser.add_argument("--mode", choices=["dev", "blind"], default="dev")
    parser.add_argument("--config", type=str, default=str(DB_DIR / "salt_paano_config.json"))
