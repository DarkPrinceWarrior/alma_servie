...
def render_stepwise_section(results: Sequence[StepwiseResult]) -> str:
    ...
    for res in ordered:
        rows.append(
            ...
            f"<td>{_format_timestamp(res.aligned_detection_start)}</td>"
            f"<td>{_format_timestamp(res.notification_time)}</td>"
            f"<td>{_format_number(res.delay_minutes, precision=1)}</td>"
            f"<td>{_format_number(res.aligned_delay_minutes, precision=1)}</td>"
    ...
    table = (
        ...
        "• <em>Старт детекции (raw)</em> — честный момент первого срабатывания без прижатия к метке.<br>"
        "• <em>Обрезанный старт</em> — старый вариант «прижатого» старта к началу метки (для сравнения).<br>"
        "• <em>Задержка (raw)</em> — может быть отрицательной, если сработали раньше окна метки.<br>"
        "• <em>Задержка (clipped)</em> — задержка после прижатия (историческое представление).<br>"
        ...
        "<th>Старт детекции (raw)</th>"
        "<th>Обрезанный старт</th>"
        "<th>Момент уведомления</th>"
        "<th>Задержка (raw), мин</th>"
        "<th>Задержка (clipped), мин</th>"
