...
Tuned for slow anomalies (days-long), similar to salt deposition.
"""
import sys
import os
import time
...
SEED = 2027
PATCH_SIZE = 64       # 64 x 2min = 128 min per patch (slow anomalies)
NUM_ITERS = 200
BATCH_SIZE = 256
LR = 1e-4
TOP_K = 3
...
def detect_pritok_paano_well(well_df, intervals_df, well_id, verbose=True):
    set_seed()
...
def find_anomaly_starts_from_scores(scores, timestamps, threshold_quantile=0.90,
                                     min_run_points=3, cooldown_hours=24):
    """Tuned for slow anomalies with high anomaly ratio: q=0.90."""
...
def run_pritok_paano_detection(output_path='pritok_paano_results.csv',
                                source_path=None, verbose=True):
    print("=== PaAno Pritok (Inflow Change) Detection ===")
    ...
    prestart_tolerance = pd.Timedelta(hours=24)
    early_status_tolerance = pd.Timedelta(hours=24)
    ...
    for wid in all_wells:
        ...
        scores, timestamps, detail = detect_pritok_paano_well(
