...
По умолчанию система читает рабочую книгу Excel `alma/Общая_таблица_новая.xlsx`, однако те же данные можно
...
├── alma/
│   └── Общая_таблица_новая.xlsx      # Входные данные (НЕ в git)
├── reports/
│   └── anomalies/
...
anomalies:
  ...
  svod_sheet: "svod"
  anomaly_causes:  # Значения из столбца ПричОст, которые считаем аномалиями
    - "Негерметичность НКТ"
  ...
  cause_profiles:        # Настройки по порогам на конкретные типы аномалий
    - name: "Негерметичность НКТ"
      group: "Негерметичность НКТ"
      include_normal: true   # учитывать нормальные интервалы при вычислении порогов
...
**Что происходит:**
- Загружается рабочая книга `alma/Общая_таблица_новая.xlsx`
