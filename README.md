...
Дополнительно поддерживается новый тип аномалии **Меха** (механические примеси/клин насоса). Для него используются отдельные датасеты `.feather`, детекция по режиму «пилы» нескольких параметров и росту температуры, а также интервальные метки из `db/meha_intervals.csv`.
...
- `db/*.feather`: Датасеты скважин для аномалии **Меха**.
- `db/meha_intervals.csv`: Интервалы ground truth для **Меха**.
...
**Примечание:** если существует `db/meha_intervals.csv`, скрипт автоматически запускает детекцию **Меха** на данных `.feather`.
